...
from generator.core.predictor.datasets.preprocess import (
    preprocess_sequence,
    scale_target,
    split_dataset,
    tokenize_sequence,
...
def test_scale_target_with_default_scaling():
    ...
    assert np.isclose(scaled_dataset[target_key], [-1.224744871391589, 0.0, 1.224744871391589]).all()  # type: ignore
...
def test_scale_target_with_custom_mean_and_scale():
    ...
    assert np.isclose(scaled_dataset[target_key], [-1.0, 0.0, 1.0]).all()  # type: ignore
