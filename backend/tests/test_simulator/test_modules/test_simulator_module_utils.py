...
from simulator.modules.utils import get_node_id2data, get_parts_name2node_ids, get_specific_category_node_ids


def test_parts_name2node_ids(test_circuit, child_ids):
    nodes = ReactFlowObject(**test_circuit).nodes

    parts_name2node_ids = get_parts_name2node_ids(nodes)

    assert {node_id for node_ids in parts_name2node_ids.values() for node_id in node_ids} == set(child_ids)
