...
import { createChildNode, createTempNode } from "@/components/Circuit/utils/create-node";
import { groupNodes, ungroupNodes } from "@/components/Circuit/utils/group-nodes";
import { NODE_HEIGHT, NODE_WIDTH, TEMP_NODE_ID, TEMP_EDGE_ID } from "@/components/Circuit/constants";
import { createNearestEdge } from "@/components/Circuit/utils/create-nearest-edge";
import { findRelatedNodes } from "@/components/Circuit/utils/related-nodes";
import { getConnectedComponents } from "@/components/Circuit/utils/connected-components";
import { adjustNodePositionsAndConnectHandles } from "@/components/Circuit/utils/adjust-nodes";
import { isNodeOutsideParent } from "@/components/Circuit/utils/node-outside-parent";
  ...
  const handleNodeDrag = useCallback(
    ...
    const handleNodeDragStop = useCallback(
      (e, node: Node) => {
        ...
        if (node.type === "parent") {
          const rightNode = nodes.filter((n) => n.parentId === node.id).reduce((acc, curr) => {
            return curr.position.x > acc.position.x ? curr : acc;
          });
          const leftNode = nodes.filter((n) => n.parentId === node.id).reduce((acc, curr) => {
            return curr.position.x < acc.position.x ? curr : acc;
          });
          ...
          const newNodes = produce(nodes, (draft) => {
            ...
            const sourceSiblingNodes = draft.filter((n) => n.type === 'child' && n.parentId === sourceNode.parentId || n.id === sourceNode.id);
            const targetSiblingNodes = draft.filter((n) => n.type === 'child' && n.parentId === targetNode.parentId || n.id === targetNode.id);
        ...
        const newNodes = produce(nodes, (draft) => {
          ...
          } else { // drag node has no parent node.
            if (nearestEdge) {
              const { sourceNode, targetNode, parentNode, siblingNodes } = findRelatedNodes(draft, nearestEdge);
