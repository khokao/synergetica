...
import icon from "@public/images/node-meta.svg";
import { fireEvent, render } from "@testing-library/react";
import { describe, expect, it } from "vitest";

describe("metaNode", () => {
  it("should render metaNode with correct image and attributes", () => {
    const { getByAltText } = render(metaNode);

    const img = getByAltText("meta");
  ...
  it("should set draggable and call onDragStart on drag start", () => {
    ...
    const { container } = render(metaNode);
    ...
    expect(mockDragEvent.dataTransfer.setData).toHaveBeenCalledWith("application/reactflow-node-type", "meta");
    expect(mockDragEvent.dataTransfer.setData).toHaveBeenCalledWith(
      "application/reactflow-icon-url",
      "/images/node-meta.svg",
    );
    expect(mockDragEvent.dataTransfer.setData).toHaveBeenCalledWith(
      "application/reactflow-left-handle-style",
      JSON.stringify({ top: 15, left: 8 }),
    );
    expect(mockDragEvent.dataTransfer.setData).toHaveBeenCalledWith(
