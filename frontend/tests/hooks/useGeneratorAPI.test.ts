import { callGeneratorAPI, fetcher } from "@/hooks/useGeneratorAPI";
...
describe("useGeneratorAPI hooks", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe("fetcher function", () => {
    it("invokes the correct key", async () => {
      // Arrange
      ...
      // Act
      const result = await fetcher(mockKey);

      // Assert
      expect(invoke).toHaveBeenCalledWith(mockKey);
    ...
    it("handles errors correctly", async () => {
      ...
      // Act & Assert
      await expect(fetcher(mockKey)).rejects.toThrow("Test error");
    });
  });
